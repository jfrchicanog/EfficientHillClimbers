...
public class NaiveHillClimberSnapshot implements
	...
	NaiveHillClimberSnapshot(
			NaiveHillClimberForInstanceOf naiveHillClimberForInstanceOf,
			Solution<? super WeightedGraphColoring> fsol) {
		...
		if (fsol instanceof WGCSolution) {
			...
			throw new IllegalArgumentException(
					"Expected an object of class WGCSOlution and got one of class "
	...
	public WGCMove getMovement() {
		...
		int[][] adjacents = this.naiveHillClimberForInstanceOf.prob
				.getAdjacents();

		double histogram[];

		for (int u = 0; u < this.naiveHillClimberForInstanceOf.prob
				.getVertices(); u++) {
			histogram = new double[colors];
			for (int v : adjacents[u]) {
				histogram[sol.colors[v]] += this.naiveHillClimberForInstanceOf.prob
						.getWeight(u, v);
			}
			...
			if (histogram[min] - histogram[sol.colors[u]] < movement.improvement) {
				movement.improvement = histogram[min]
