...
public class NKLandscapes extends EmbeddedLandscape implements
    ...
    public void setConfiguration(Properties prop) {
        ...
        if (isSat) {
//            m = (int) (3 * n);
//            We are generating sub functions with 3 zeroes which means one sub function is superposition of three sub functions
            m = n;
        }
...
//             Generating subfunctions considering alpha in the problem setup phase
//            numbers = Stream.concat(
...
//                    )
//                    .collect(Collectors.toList());
//            alpha is considered in evaluation phase
//            numbers = Stream.concat(
//                            (twoToK > 1) ? IntStream.range(1, twoToK).mapToDouble(i -> sat_step).boxed()
//                                    : Stream.of(sat_step),
//                            Stream.of(0.0) // Single zero added
//                    )
//                    .collect(Collectors.toList());
            numbers = Stream.concat(
                            (twoToK > 1) ? IntStream.range(0, Math.max(0, twoToK - 3)).mapToDouble(i -> sat_step).boxed()
                                    : Stream.of(sat_step),
                            Stream.of(0.0, 0.0, 0.0) // Single zero added
                    )
                    .collect(Collectors.toList());
...
//                        .toArray();
                subFunctions[sf] = IntStream.range(0, twoToK)
                        .mapToDouble(i -> (q > 0) ? rnd.nextInt(q) - shift : rnd.nextDouble())
                        .toArray();
            }
...
//        int start=0, end=m-1;
//        while(start <= end){
//            int temp = masks[start][k-1];
//            int temp2 = masks[end][k - 1];
//            while(isExist(masks[start], temp2)){
//                temp2 = (temp2 == 0) ? 1 : temp2-1;
//            }
//            masks[start][k-1] = temp2;
//
//            while(isExist(masks[end], temp)){
...
//        }
        ...
        while (!valid && attempt < maxAttempts) {
            ...
            // Shuffle each column in-place
            for (int col = 0; col < cols; col++) {
                // Fisher–Yates shuffle for each column
                for (int i = rows - 1; i > 0; i--) {
                    int j = rand.nextInt(i + 1);
                    // Swap masks[i][col] and masks[j][col]
                    int temp = masks[i][col];
                    masks[i][col] = masks[j][col];
                    masks[j][col] = temp;
                }
            }
    ...
    private boolean isExist(int[] mask, int val){
        for (int j : mask) {
            if (val == j) {
    ...
    public double evaluate(Solution sol) {
        ...
        for (int sf = 0; sf < m; sf++) {
            ...
            res += (subFunctions[sf][index] * alpha);
        }
    ...
    public double evaluateSubfunction(int sf, PBSolution pbs) {
        ...
        return alpha * subFunctions[sf][index];
    }
    ...
    public double evaluateSubfunction(int sf, int value) {
        ...
        return alpha * subFunctions[sf][value];
    }
    ...
    public void writeTo(Writer wr) {
        ...
        pw.println("c The next line contains N, K, Q and m");
        pw.println("p NK " + n + " " + (k - 1) + " " + q + " " + m);
