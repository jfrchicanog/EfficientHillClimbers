...
public class MAXSAT extends EmbeddedLandscape implements EmbeddedLandscapeSubfunctionMaximizer {
	...
	public static final String MIN_K_STRING = "min_k";
	...
	public void setConfiguration(Properties prop) {
		...
		if (prop.getProperty(INSTANCE_STRING) != null) {
			...
			int minK = 1;
			if (prop.getProperty(MIN_K_STRING) != null) {
				minK = Integer.parseInt(prop.getProperty(MIN_K_STRING));
			}
			generateRandomInstance(n, m, maxK, minK);
		}
	...
	private void generateRandomInstance(int n, int m, int max_k, int min_k) {
		if (min_k == 0) {
			throw new IllegalArgumentException("minimum k must be greater than zero");
		}
		if (max_k < min_k) {
			throw new IllegalArgumentException("maximum k cannot be less than minimum k");
		}
		...
		for (int c = 0; c < m; c++) {
			int k = (min_k==max_k)?min_k:(rnd.nextInt(max_k-min_k+1) + min_k);
			// masks[c] = new int [k];
			clauses[c] = new int[k];
    ...
    public void writeInCNFFormat(Writer wr) {
    	PrintWriter pw = new PrintWriter(wr);
    	pw.println("c MAX-SAT instance in CNF format");
		pw.println("c Generated by " + getClass().getName());
		pw.println("c Random seed: " + seed);
		pw.println("p cnf "+n+" "+m);
		for (int clause=0; clause < m; clause++) {
			for (int literal=0; literal < clauses[clause].length; literal++) {
				pw.print(clauses[clause][literal]+" ");
			}
			pw.println("0");
		}
    }
	...
	public void writeTo(Writer wr) {
		...
		pw.println("c MAX-SAT instance in Brian Goldman format to enumerate all Local Optima");
		pw.println("c Generated by " + getClass().getName());
		pw.println("c The next line contains n");
		pw.println("p MK " + n);
		pw.println("c For each subfunction first it appears the mask in a line starting with 'm'");
		pw.println("c The mask is a list of values with the indices of the variables in Big Endian order");
		...
		for (int i = 0; i < m; i++) {
			...
			pw.print("m ");
			int zeroSolution = 0;
			for (int j = getMaskLength(i) - 1; j >= 0; j--) {
				pw.print(getMasks(i, j) + " ");
				zeroSolution = (zeroSolution << 1) | ((clauses[i][j] < 0)?1:0);
			}
			pw.println();

			// Write the subfunction values
			for (int j = 0; j < (1 << getMaskLength(i)); j++) {
				pw.print(((zeroSolution==j)?0:1) + " ");
			}
			pw.println();
		}
