...
//			int k = rnd.nextInt(max_k) + 1;
	...
	public void writeTo(Writer wr) {
		...
		pw.println("===============================================================================================");
		pw.println("c MAXSAT instance");
		pw.println("c Generated by " + getClass().getName());
		pw.println("c The next line contains N, m and maxK");
		pw.println("p NK " + n + " " + m + " " + maxK);
		pw.println("c For each clauses first it appears the mask in a line starting with 'm'");
		pw.println("c The mask is a list of upto maxK values with the indices of the variables in small Endian order");
		pw.println("c Following the mask the clauses is defined in a line without prefix");
