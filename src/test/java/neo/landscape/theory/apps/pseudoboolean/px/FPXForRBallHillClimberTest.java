...
public class FPXForRBallHillClimberTest {
    
    private NKLandscapes nk;
    private RBallCrossoverAdaptor rballDPX;
    private FourierPartitionCrossover<WalshCoefficientsArray> fpx;
    private Random rnd;
    private double alpha = 0.15;

    @Test
    public void test() {
        IntStream.range(3, 10).map(n->n*10).forEach(n->{
            int k=2;
            int q = 2;
            int pseed = 0;
    ...
    private void testSolutionPair(PBSolution red, PBSolution blue) {
        FourierPartitionCrossoverConfigurator fpxConfig = new FourierPartitionCrossoverConfigurator();
        fpx = (FourierPartitionCrossover<WalshCoefficientsArray>) fpxConfig.configureCrossover(null, nk, null);
        rballDPX = new RBallCrossoverAdaptor(fpxConfig.configureCrossover(null, nk, null));
        
        PBSolution result = fpx.recombine(blue, red);
