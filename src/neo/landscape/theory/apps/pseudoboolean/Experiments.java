...
public class Experiments {
	...
	{
		System.out.println("Arguments: maxsat [-h] [-i <instance>] [-r <r>] [-ma(xsat specific HC)] [-l <quality> <limits> ...] [-t <time(s)>] [-d <descents>] [-s <soft_restart>] [-se <seed>] [-tr(ace)] [-de(bug)] [-fl(ips) vs appearance] [-hp(hyperplane initialization, only for cnf instances)]");
	}
	...
	public void showMaxsatHpInitHelp()
	{
		System.out.println("Arguments: maxsatHpInit [-h] [-i <instance>] [-r <r>] [-ma(xsat specific HC)] [-l <quality> <limits> ...] [-hp(hyperplane initialization)] [-d <descents>] [-se <seed>] [-de(bug)] [-fl(ips) vs appearance]");
	}
	
	public void maxsatHpInitExperiments(String[] args) {
		if (args.length == 0)
		{
			showMaxsatHelp();
			return;
		...
		// Check the descents
		if (!checkOneValue(options, "d", "descents")) return;
		// else
