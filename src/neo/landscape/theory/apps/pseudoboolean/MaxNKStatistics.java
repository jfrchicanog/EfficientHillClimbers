 ...
 * @email chicano@lcc.uma.es This class should compute the probability
 *        distribution of the maximum value for an NK Landscape. However, I
 *        think the implementation is wrong. It could be also the case that this
 *        computation is used only for the adjacent model. Definitely, I think
 *        th distribution is built for the K=1 adjacent model.
 */
...
public class MaxNKStatistics {
	...
	public void computeDistribution() {
		...
		for (int f = 2; f <= n; f++) {
			for (int j = f * (q - 1) - 1; j >= 0; j--) {
				...
				// of a sum of two random variables where one is uniformly
				// distributed with q values
				double sum = 0;
				for (int k = j; k > Math.max(-1, j - q); k--) {
					sum += distribution[k];
				}
				sum /= q;
				// The square has the effect of computing the probability
				// distribution of
				// the maximum of two random variables distributed with the
	...
	public static void main(String[] args) {
		...
		ms.setConfiguration(Integer.parseInt(args[0]),
