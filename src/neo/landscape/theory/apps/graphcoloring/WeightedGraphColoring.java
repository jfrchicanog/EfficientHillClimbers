...
public class WeightedGraphColoring implements Problem {
	...
	public static final String [] colorNames = {"blue", "red", "green", "purple", "yellow", "white", "black"};
	...
	public <P extends Problem> double evaluate(Solution<P> sol) {
		if (sol instanceof WGCSolution)
		{
			...
			throw new IllegalArgumentException("Expected a solution of class WGCSolution but obtained one of class "+sol.getClass().getCanonicalName());
	}
	...
	public String dotLanguage (WGCSolution sol, WGCMove m)
	{
		...
		for (int i = 0; i < sol.colors.length; i++) {
			...
			{
				str += vertexName(i,m)+" [shape=doublecircle style=filled color="+ colorString(sol.colors[i])+" fontcolor=" +colorString(m.color)+"];\n";
			}
			else
			{
				str += "\""+i + "\" [style=filled color="+ colorString(sol.colors[i])+"];\n";
			}
		}
